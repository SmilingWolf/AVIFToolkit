...
vpxenc --profile=1 -v --row-mt=0 --tile-rows=0 --tile-columns=0 --frame-parallel=0 --threads=1 --cpu-used=0 --end-usage=q --cq-level=$quality --sharpness=7 --color-space=bt709 --ivf -o "encoded/$filename.ivf" "temp/orig_$filename.y4m" 2> /dev/null
./bins/MP4Box -add "encoded/$filename.ivf" "contained/$filename.mp4" 2> /dev/null
...
./bins/ffmpeg -y -hide_banner -loglevel fatal -i "encoded/$filename.ivf" -vf scale=flags=lanczos+accurate_rnd+bitexact+full_chroma_int+full_chroma_inp,format=bgr24 -strict -1 "temp/enc_$filename.png"
