...
./bins/ffmpeg -r 1 -y -hide_banner -loglevel fatal -i "$1" -vf $FFpad,scale=in_color_matrix=bt709:flags=lanczos+accurate_rnd+bitexact+full_chroma_int+full_chroma_inp,format=bgr24 -strict -1 "temp/orig_$filename.bmp"
./bins/cjpeg -quality $quality $sampleSize -outfile "encoded/$filename.jpeg" "temp/orig_$filename.bmp"
